...
namespace SustitucionMOAUtils.Validadores.OrdenDeCarga
{
	public class ValidarCorredorClienteContratoProductoRequestValidator : AbstractValidator<ValidarCorredorClienteContratoProductoRequest>
	{
		...
		{
			RuleFor(dto => dto.ClienteCuit).NotNull().NotEmpty().OnAnyFailure(error => throw new ValidationCustomException(string.Format(ErrorMsg.ErrorValorNuloVacio, "CUIT Cliente")));
			...
			RuleFor(dto => dto.ProductoId).NotNull().NotEmpty().OnAnyFailure(error => throw new ValidationCustomException(string.Format(ErrorMsg.ErrorValorNuloVacio, "Id Producto")));
		}
	}
}
