...
export class AltasComponent extends BaseComponent implements OnInit {
    ...
    deshabilitarUsuario(estadoId: number) {
        if (this.observacionesProveedor == "") {
            this.mensajeError = "Debe ingresar una observacion para el Proveedor.";
            return false;
        }
        this.spinnerComponent.showIt();
        this.mensajeComponent.setMsgsEmpty();
        try {
            this.altaEmpresaService.deshabilitarUsuario(this.empresaSeleccionada.Id, this.observaciones, this.observacionesProveedor).subscribe(
                result => {
                    this.getEmpresa();
                    this.spinnerComponent.hideIt();
                    if (result.logout == true) {
    ...
    habilitarUsuario(estadoId: number) {
        this.spinnerComponent.showIt();
        this.mensajeComponent.setMsgsEmpty();
        try {
            this.altaEmpresaService.habilitarUsuario(this.empresaSeleccionada.Id, this.observaciones).subscribe(
                result => {
                    this.getEmpresa();
                    this.spinnerComponent.hideIt();
                    if (result.logout == true) {
