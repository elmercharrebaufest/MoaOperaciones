...
namespace SustitucionMOA.Controllers
{
    public class OrdenDeCargaController : BaseController
    {
        ...
        [HttpGet]
        public ActionResult GetListadoFiltradoOrdenCarga(string fechaInicio, string fechaFin)
        {
            try
            {
                var mailUsuario = SessionPersister.getUsername();

                return JsonCustom(new { data = ordenDeCargaService.ListadoFiltradoOrdenCarga(mailUsuario, fechaInicio, fechaFin)});
            }
            catch (InfoCustomException e)
            {
                return Json(new { info = e.Message }, JsonRequestBehavior.AllowGet);
