...
@Component({
  selector: 'app-filtro-fecha-reporte',
  templateUrl: './filtro-fecha-reporte.component.html'
})
export class FiltroFechaReporteComponent implements OnInit {

  mostrarPendientes: boolean = true;
  @Output() FiltroPendientes = new EventEmitter();
  ...
  enviarFiltroPendientes(){
    this.FiltroPendientes.emit(this.mostrarPendientes);
  }
  filterByDate() {

    let fechaHoraInicio = (document.getElementById('dtp_input1') as HTMLInputElement).value;
    let fechaHoraFin = (document.getElementById('dtp_input2') as HTMLInputElement).value;
