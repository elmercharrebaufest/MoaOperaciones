...
DECLARE @rolIdSolp INT = (SELECT TOP 1 Id FROM dbo.Rol WHERE Nombre = 'ACCESO QR')
...
BEGIN
	...
	VALUES
	(   N'ACCESO QR', -- Codigo - nvarchar(max)
	    N'Acceso QR', -- Nombre - nvarchar(max)
	    1 -- EsEditable - bit
	)
...
IF NOT EXISTS (SELECT TOP 1 1 FROM dbo.PermisoPorRol WHERE Permiso = 'ACCESO QR')
BEGIN
	...
	VALUES
	(N'ACCESO QR' -- Permiso - nvarchar(max)
	)
